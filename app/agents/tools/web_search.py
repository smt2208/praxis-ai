...
Features:
  - Real-time web retrieval and live citations directly via OpenAI Responses API.
  - Low-latency query execution powered by FAST_MODEL.
  - Cached sync/async OpenAI client singletons to maximize HTTP connection reuse.
  - Resilient fallback to Chat Completions API if Responses API encounters transient issues.
"""
import logging
