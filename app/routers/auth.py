...
from app.db import (
    ...
    update_user_profile, get_memory_enabled,
)
...
async def login(request: Request, body: LoginRequest, pool: asyncpg.Pool = Depends(get_pool)):
    ...
    is_valid = await asyncio.to_thread(verify_password, body.password, user["hashed_password"])
    if not is_valid:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Incorrect email or password.",
...
async def reset_password(body: ResetPasswordRequest, pool: asyncpg.Pool = Depends(get_pool)):
    ...
    new_hashed = await asyncio.to_thread(hash_password, body.new_password)
